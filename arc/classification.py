...
class ProbabilityAccumulator:
    ...
    def predict_sets(self, alpha, randomize=False, epsilon=None):
        J = np.argmax(self.Z >= 1.0-alpha, axis=1).flatten()
        if randomize:
            epsilon = np.random.uniform(low=0.0, high=1.0, size=self.n)
        if epsilon is not None:
            Z_excess = np.array([ self.Z[i, J[i]] for i in range(self.n) ]) - (1.0-alpha)
            p_remove = Z_excess / np.array([ self.prob_sort[i, J[i]] for i in range(self.n) ])
            remove = epsilon <= p_remove
            #pdb.set_trace()
            for i in np.where(remove)[0]:
                J[i] = J[i] - 1
        # Return prediction set
        S = [ self.order[i,np.arange(0, J[i]+1)] for i in range(self.n) ]
        return(S)
